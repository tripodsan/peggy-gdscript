...
function generateGD(ast, options) {
  ...
  function generateTables() {
    ...
    function buildRegexp(cls) {
      ...
      return `PEG.RegExp.new("${stringEscape(pattern)}")`;
    }
    ...
    function buildExpectation(e) {
      switch (e.type) {
        case "rule": {
          return "_otherEx(\"" + stringEscape(e.value) + "\")";
        }
        case "literal": {
          return "_litEx(\""
  ...
  function generateRuleHeader(ruleNameCode, ruleIndexCode) {
    ...
    if (options.cache) {
      parts.push(
        ...
        "var cached:CacheEntry = peg_resultsCache.get(key)",
        "if (cached):",
  ...
  function generateRuleFooter(ruleNameCode, resultCode) {
    ...
    if (options.cache) {
      parts.push(
        `peg_resultsCache[key] = CacheEntry.new(peg_currPos, ${resultCode})`
      );
    }
      ...
      while (ip < end) {
        switch (bc[ip]) {
          ...
          case op.TEXT:               // TEXT
            const pos = stack.pop();
            parts.push(
              stack.push(`input.substr(${pos}, peg_currPos - ${pos})`)
            );
            ip++;
            break;
          ...
          case op.IF_LT:              // IF_LT min, t, f
            compileCondition(`len(${stack.top()}) < ${bc[ip + 1]}`, 1);
            break;
          ...
          case op.IF_LT_DYNAMIC:      // IF_LT_DYNAMIC min, t, f
            compileCondition(`len(${stack.top()}) < (${stack.index(bc[ip + 1])}|0)`, 1);
            break;
          ...
          case op.MATCH_ANY:          // MATCH_ANY a, f, ...
            compileCondition("len(input) > peg_currPos", 0);
            break;
          ...
          case op.MATCH_STRING: {     // MATCH_STRING s, a, f, ...
            ...
            compileInputChunkCondition(
              (inputChunk, optimized) => {
                ...
                inputChunk = !optimized
                  ? "(input.unicode_at(peg_currPos) if peg_currPos < len(input) else 0)"
          ...
          case op.MATCH_STRING_IC: {  // MATCH_STRING_IC s, a, f, ...
            const litNum = bc[ip + 1];
            compileInputChunkCondition(
              inputChunk => `${inputChunk}.to_lower() == ${l(litNum)}`,
              1,
          ...
          case op.FAIL:               // FAIL e
            parts.push(stack.push("peg_FAILED"));
            parts.push(`if (peg_silentFails == 0): _fail(${e(bc[ip + 1])})`);
            ip += 2;
            break;
  ...
  function generateToplevel() {
    const parts = [];
    let template = readFileSync(options.parserTemplate, "utf-8");
    template = template.replace("__VERSION__", version);
    template = template.replace("__CLAZZ_NAME__", options.className || "Parser");
    template = template.replace("##GLOBALS##", ast.topLevelInitializer?.code || "");
    ...
    template = template.replace("##START_RULES##", start.join("\n"));
    /*
  ...
  if options.has('startRule'):
    ...
    ast.rules.forEach(rule => {
      expressions.push(...generateRuleFunction(rule));
    });
